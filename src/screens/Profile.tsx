...
export function Profile(){
    ...
    try{
      ...
      if(photoSelected.assets[0].uri){
        ...
        if(photoInfo.size && (photoInfo.size / 1024 / 1024) > 5){
          toast.show({title: "Essa imagem é muito grande. Escolha uma de até 5MB", placement: "top", bgColor: "red.500"})
          return;
        }
        ...
        toast.show({title: "Foto atualizada!", placement: "top", bgColor: "green.700"})
      }
    }catch(e){
      console.log(e)
    }finally{
  ...
  async function handleProfileUpdate(data: FormDataProps) {
    try{
      ...
      toast.show({title: "Perfil atualizado com sucesso!", placement: "top", bgColor: 'green.500'})
    }catch(error){
      const isAppError = error instanceof AppError;
      const title = isAppError ? error.message : 'Não foi possível atualizar os dados. Tente novamente mais tarde';
      toast.show({title, placement: "top", bgColor: 'gredreen.500'})
        ...
        <ScrollView contentContainerStyle={{paddingBottom: 56}}>
          <Center mt={6} px={10}>
            ...
            />
              ...
              <Heading color="gray.200" mt={12} fontSize="md" mb={2} alignSelf="flex-start"  fontFamily={"heading"}>
                Alterar Senha
              </Heading>
